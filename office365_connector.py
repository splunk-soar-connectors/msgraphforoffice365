...
class Office365Connector(BaseConnector):
    ...
    def _handle_tmp(self, param):
        ...
        Write-Host "attempting to connect to ExchangeOnline"

        Connect-ExchangeOnline -AccessToken $token -AppId "{self._client_id}" -Organization "{self._tenant}"

        if ($?) {{
            Write-Host "connected to ExchangeOnline"

            Get-MessageTrace
            Disconnect-ExchangeOnline -Confirm:$false
            exit
        }} else {{
            Write-Host "unable to connect to ExchangeOnline"
            exit
        }}

        Write-Host "action ended"
        """

        output = subprocess.run(["pwsh", "-Command", ps_command], capture_output=True, text=True)
        self.save_progress(f"output: {output}")
        self.save_progress(f"output.stdout: {output.stdout}")
        self.save_progress(f"output.stderr: {output.stderr}")
